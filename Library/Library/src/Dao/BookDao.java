...
public class BookDao extends DBConnect {
    ...
    public ArrayList<Book> getAllBook(){
        ...
        try {
            ...
            while (rs.next()) {
                ...
                book.setId(rs.getString("id"));//数据库
    ...
    public ArrayList<Book> getadd(){
        ...
        try {
            ...
            ResultSet rs = null;
            pst = conn.prepareStatement(sql);
            rs = pst.executeQuery();//executeQuery()方法会把数据库响应的查询结果存放在ResultSet类对象中供我们使用。即语句：ResultSet rs=s.executeQuery(sql);
            Book addbook = null;
            while (rs.next()) {
                addbook = new Book();
